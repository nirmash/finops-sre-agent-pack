...
2. Step 1 (pull): GET Consumption UsageDetails (ActualCost) for the last 35 days via \`az rest --method get\` in **5-day date-windowed slices** (\`&\$filter=properties/usageStart ge 'A' and properties/usageStart lt 'B'\`, \`&\$top=1000\`, paginate nextLink within each slice, concatenate) — this avoids the deep-pagination 413 Request Too Large that persists even at \$top=20 over a long range; if a slice still 413s, halve it (~2-3 days) and drop \$top (1000→100→20). Flatten each row to {date, cost, meterCategory, resourceGroup, resourceId, tags} (resourceId from properties.instanceName, falling back to properties.resourceId — resourceId is null in modern billing). If a slice cannot complete, keep partial rows but label downstream totals "partial — cost pull truncated".
...
6. Step 4 (cost): GET Consumption UsageDetails (ActualCost) for ~30 days via \`az rest --method get\` in **5-day date-windowed slices** (\`&\$filter=properties/usageStart ge 'A' and properties/usageStart lt 'B'\`, \`&\$top=1000\`, paginate nextLink within each slice, concatenate) to avoid the deep-pagination 413 (persists even at \$top=20 over a long range); if a slice still 413s, halve it and drop \$top (1000→100→20). Aggregate costInUSD by resourceId into {resourceId: monthly_usd}. If a slice cannot complete, keep partial rows but label savings totals "partial — cost pull truncated".
...
2. Pull the data NOW, at authoring time, using read-only Azure cost commands. Use \`az rest --method get\` against Consumption UsageDetails (ActualCost) for the last 30 days in **5-day date-windowed slices** (\`&\$filter=properties/usageStart ge 'A' and properties/usageStart lt 'B'\`, \`&\$top=1000\`, paginate nextLink within each slice, concatenate) to avoid the deep-pagination 413 Request Too Large (persists even at \$top=20 over a long range); if a slice still 413s, halve it (~2-3 days) and drop \$top (1000→100→20). If a slice cannot complete, keep partial rows but label the report totals "partial — cost pull truncated". Do NOT use \`az rest --method post\` or the Cost Management Query API — POST is blocked as a write. Read costInUSD; take the resource id from properties.instanceName, falling back to properties.resourceId (resourceId is null in modern billing).
