...
2. Step 1 (pull): GET Consumption UsageDetails (ActualCost) for the last 35 days via \`az rest --method get\` in **3-day date-windowed slices** (\`&\$filter=properties/usageStart ge 'A' and properties/usageStart lt 'B'\`, \`&\$top=1000\`, paginate nextLink within each slice, concatenate). Project to just the needed fields with \`--query "{value: value[].{date: properties.date, cost: properties.costInUSD, meterCategory: properties.meterCategory, resourceGroup: properties.resourceGroup, resourceId: properties.instanceName, tags: tags}, nextLink: nextLink}"\` so the retained JSON stays small. The 413 Request Too Large is a server-side response-size limit — short 3-day slices + bounded \$top keep each page under the cap (a 5-day slice trips it); --query is client-side and does NOT prevent the 413. If a slice still 413s, halve it (~1 day) and drop \$top (1000→100→20). nextLink in the body is HTML-escaped (&amp;) — decode before following. resourceId comes from properties.instanceName, falling back to properties.resourceId (resourceId is null in modern billing). If a slice cannot complete, keep partial rows but label downstream totals "partial — cost pull truncated".
...
6. Step 4 (cost): GET Consumption UsageDetails (ActualCost) for ~30 days via \`az rest --method get\` in **3-day date-windowed slices** (\`&\$filter=properties/usageStart ge 'A' and properties/usageStart lt 'B'\`, \`&\$top=1000\`, paginate nextLink within each slice, concatenate), projecting to just the needed fields with \`--query "{value: value[].{date: properties.date, cost: properties.costInUSD, resourceGroup: properties.resourceGroup, resourceId: properties.instanceName, tags: tags}, nextLink: nextLink}"\`. The 413 is a server-side response-size limit — short 3-day slices + bounded \$top keep each page under the cap (a 5-day slice trips it); --query is client-side and does NOT prevent it. If a slice still 413s, halve it (~1 day) and drop \$top (1000→100→20). Aggregate costInUSD by resourceId into {resourceId: monthly_usd}. If a slice cannot complete, keep partial rows but label savings totals "partial — cost pull truncated".
...
2. Pull the data NOW, at authoring time, using read-only Azure cost commands. Use \`az rest --method get\` against Consumption UsageDetails (ActualCost) for the last 30 days in **3-day date-windowed slices** (\`&\$filter=properties/usageStart ge 'A' and properties/usageStart lt 'B'\`, \`&\$top=1000\`, paginate nextLink within each slice, concatenate), projecting to just the needed fields with \`--query "{value: value[].{date: properties.date, cost: properties.costInUSD, meterCategory: properties.meterCategory, resourceGroup: properties.resourceGroup, resourceId: properties.instanceName, tags: tags}, nextLink: nextLink}"\`. The 413 Request Too Large is a server-side response-size limit — short 3-day slices + bounded \$top keep each page under the cap (a 5-day slice trips it); --query is client-side and does NOT prevent it. If a slice still 413s, halve it (~1 day) and drop \$top (1000→100→20). If a slice cannot complete, keep partial rows but label the report totals "partial — cost pull truncated". Do NOT use \`az rest --method post\` or the Cost Management Query API — POST is blocked as a write. Read costInUSD; take the resource id from properties.instanceName, falling back to properties.resourceId (resourceId is null in modern billing).
