...
2. Step 1 (pull): GET Consumption UsageDetails (ActualCost) for the last 35 days via \`az rest --method get\` with a bounded page size \`&\$top=100\` (retry smaller — 50, then 20 — on a 413 Request Too Large; do NOT re-add \$top when following nextLink), paginate nextLink, and flatten each row to {date, cost, meterCategory, resourceGroup, resourceId, tags} (resourceId from properties.instanceName, falling back to properties.resourceId — resourceId is null in modern billing). If pagination cannot complete, keep partial rows but label downstream totals "partial — cost pull truncated".
...
6. Step 4 (cost): GET Consumption UsageDetails (ActualCost) for ~30 days via \`az rest --method get\` with a bounded page size \`&\$top=100\` (retry smaller — 50, then 20 — on a 413 Request Too Large), paginate nextLink, aggregate costInUSD by resourceId into {resourceId: monthly_usd}. If pagination cannot complete, keep partial rows but label savings totals "partial — cost pull truncated".
